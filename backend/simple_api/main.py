...
def composite_wig(user_photo_url: str, wig_filename: str,
    ...
    # Female styles need a small positive margin: fy is the top of the
    # *face* bbox (around brow/hairline), but the user's actual scalp
    # sits a bit above that — without any margin the wig's crown lands
    # right at fy and a sliver of the original hairline shows above it.
    # 0.35 * fh overshot past the real top of the head entirely; 0.12 is
    # closer to the actual forehead-to-hairline distance.
    crown_margin  = round(fh * 0.12) if is_female_style else 0
